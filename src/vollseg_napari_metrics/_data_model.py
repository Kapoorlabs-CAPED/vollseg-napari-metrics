...
class pandasModel(QtCore.QAbstractTableModel):
            ...
            if role == QtCore.Qt.EditRole:
                ...
                if isinstance(v, np.float64):
                    try:
                        ...
                        print("  No action -->> please enter a number")
    ...
    def myCopyTable(self):
        ...
        print(f"Copied table to clipboard with shape: {dfCopy.shape}")
        pprint(dfCopy)
    ...
    def old_myGetValue(self, rowIdx, colStr):
        val = None
        if colStr not in self._data.columns:
            print(f'Got bad column name: "{colStr}"')
        elif len(self._data) - 1 < rowIdx:
            print(f"Got bad row:{rowIdx} from possible {len(self._data)}")
        else:
